    ...
    def generate_waymo_type_results(self, infos, class_names, is_gt=False, fake_gt_infos=True):
        ...
        for frame_index, info in enumerate(infos):
            if is_gt:
                ...
                boxes = info['gt_boxes_lidar'][box_mask]
                boxes3d.append(boxes[:,[0,1,3,4,6]])
            else:
                num_boxes = len(info['boxes_lidar'])
                difficulty.append([0] * num_boxes)
                score.append(info['score'])
                boxes = np.array(info['boxes_lidar'])
                boxes3d.append(boxes[:,[0,1,3,4,6]])
                box_name = info['name']
...
def main():
    ...
    parser.add_argument('--pred_infos', type=str, default='/data/xqm/click2box/Fast_det/output/waymo_models/pointpillar_1x/default/eval/epoch_2/val/default/result.pkl', help='pickle file')
    parser.add_argument('--gt_infos', type=str, default='/data/xqm/click2box/Fast_det/data/waymo/waymo_processed_data_v0_5_0_infos_val.pkl', help='pickle file')
