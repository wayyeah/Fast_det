...
__device__ inline int check_in_box2d_input3d(const float *box, const Point &p){
...
__device__ inline float box_overlap(const float *box_a, const float *box_b, const int input_2d=1){
...
__device__ inline float iou_3d(const float *box_a, const float *box_b){
    // params: box_a/box_b (7)  [x1, y1, z1, x2, y2, z2, angle]
    float va = (box_a[3] - box_a[0]) * (box_a[4] - box_a[1]) * (box_a[5] - box_a[2]);
    float vb = (box_b[3] - box_b[0]) * (box_b[4] - box_b[1]) * (box_b[5] - box_b[2]);
    float min_height = fmaxf(box_a[2], box_b[2]);
    float max_height = fminf(box_a[5], box_b[5]);
    float delta_height = fmaxf(max_height - min_height, EPS);
    if(delta_height == EPS){
        return 0.0;
    }
    float v_overlap = box_overlap(box_a, box_b, 0) * delta_height;
    return v_overlap / fmaxf(va + vb - v_overlap, EPS);
}
...
__global__ void boxes_iou_3d_kernel(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_iou){
    ...
    float cur_iou_3d = iou_3d(cur_box_a, cur_box_b);
...
__global__ void nms_3d_kernel(const int boxes_num, const float nms_overlap_thresh,
                           const float *boxes, unsigned long long *mask){
    //params: boxes (N, 7) [x1, y1, z1, x2, y2, z2, ry]
    //params: mask (N, N/THREADS_PER_BLOCK_NMS)
    ...
    if (threadIdx.x < row_size) {
        ...
        for (i = start; i < col_size; i++) {
            if (iou_3d(cur_box, block_boxes + i * 7) > nms_overlap_thresh){
                t |= 1ULL << i;
            }
        }
        const int col_blocks = DIVUP(boxes_num, THREADS_PER_BLOCK_NMS);
...
void boxesiou3dLauncher(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b, float *ans_iou){
...
void nms3dLauncher(const float *boxes, unsigned long long * mask, int boxes_num, float nms_overlap_thresh){
    dim3 blocks(DIVUP(boxes_num, THREADS_PER_BLOCK_NMS),
                DIVUP(boxes_num, THREADS_PER_BLOCK_NMS));
    dim3 threads(THREADS_PER_BLOCK_NMS);
    nms_3d_kernel<<<blocks, threads>>>(boxes_num, nms_overlap_thresh, boxes, mask);
}
