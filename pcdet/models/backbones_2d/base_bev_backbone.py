...
from .map_to_bev.bev_convS import CBAM_BLOCK
class BaseBEVBackboneCBAM(nn.Module):
...
class BaseBEVBackboneCMAB(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
        if input_channels is None:
            ...
            if len(upsample_strides) > 0:
                ...
                if stride > 1 or (stride == 1 and not self.model_cfg.get('USE_CONV_FOR_NO_STRIDE', False)):
                    self.deblocks.append(nn.Sequential(
                        ...
                        CBAM_BLOCK(num_upsample_filters[idx]),
                        nn.BatchNorm2d(num_upsample_filters[idx], eps=1e-3, momentum=0.01),
                        nn.ReLU()
                    ))
                else:
...
class BaseBEVBackboneExport(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
        if input_channels is None:
...
class BaseBEVBackboneWise(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
...
class BaseBEVBackboneXception(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
        if input_channels is None:
