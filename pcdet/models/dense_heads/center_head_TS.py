...
class CenterHeadTS(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        centerhead_weights = {k.replace('densehead.', ''): v for k, v in checkpoint.items() if 'densehead' in k}
        self.load_state_dict(centerhead_weights , strict=False)
    ...
    def forward(self, data_dict):
        ...
        for key, value in t.items():
            if key=='heatmaps' :
                x[key] = t[key][0].cpu().detach().numpy()
